/* vendored version of fresh-logging with updated dependencies
...
export function getLogger(options?: LoggingOpts): MiddlewareHandler {
  ...
  const remoteIpResolver = (
    _req: Request,
    ctx: MiddlewareHandlerContext,
    _res: Response,
  ) => (ctx.remoteAddr as Deno.NetAddr).hostname;

  const durationResolver = async (
    _req: Request,
    ctx: MiddlewareHandlerContext,
  ) => {
    const [duration, res] = await _durationCalculator(() => ctx.next()) as [
      string,
      Response,
    ];
    let durationText = "";
  ...
  const commonLogPartsResolver = async (
    req: Request,
    ctx: MiddlewareHandlerContext,
  ) => {
    const [durationText, res] = await durationResolver(req, ctx) as [
      string,
      Response,
    ];
    const logParts = [
  ...
  switch (format) {
    case LoggingFormat.COMMON:
      return async (
        ...
        const [durationText, res, commonLogParts] =
          await commonLogPartsResolver(req, ctx) as [
            string,
            Response,
            string[],
          ];
    ...
    case LoggingFormat.APACHE_COMBINED:
      return async (
        ...
        const logParts = commonLogParts
          .concat(
            combinedHeaders.slice(0, 2).map((header) =>
              headerResolver(req, ctx, res, header)
            ),
          )
          .concat([durationText]);
        logger(logParts.join(" "));
